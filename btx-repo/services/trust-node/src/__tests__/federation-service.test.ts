...
// ---------- mock repository ----------
const mockRepo = {
  recordPeerSync: vi.fn(),
  markPeerSyncFailed: vi.fn(),
  markPeerSyncVerified: vi.fn(),
  updateSyncCursor: vi.fn(),
  getState: vi.fn(),
  getPendingSyncs: vi.fn(),
  initializeFederationState: vi.fn(),
} as unknown as FederationRepository;

const mockKmsVerify = vi.fn<Promise<boolean>, [string, string, string]>();
...
describe('FederationService', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockRepo.recordPeerSync = vi.fn().mockResolvedValue(baseSyncEvent);
    mockRepo.markPeerSyncFailed = vi.fn().mockResolvedValue(undefined);
    mockRepo.markPeerSyncVerified = vi.fn().mockResolvedValue(undefined);
    mockRepo.updateSyncCursor = vi.fn().mockResolvedValue(undefined);
  ...
  describe('getState()', () => {
    it('returns the federation state for a known node', async () => {
      ...
      mockRepo.getState = vi.fn().mockResolvedValue(state);
    ...
    it('returns null for an unknown node', async () => {
      mockRepo.getState = vi.fn().mockResolvedValue(null);
  ...
  describe('getPendingSyncs()', () => {
    it('returns list of pending sync events', async () => {
      ...
      mockRepo.getPendingSyncs = vi.fn().mockResolvedValue(pending);
    ...
    it('returns empty array when no pending syncs', async () => {
      mockRepo.getPendingSyncs = vi.fn().mockResolvedValue([]);
      const result = await service.getPendingSyncs();
