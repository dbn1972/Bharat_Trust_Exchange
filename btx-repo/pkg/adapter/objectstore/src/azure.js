...
export function createAzureBlobStore(cfg) {
    ...
    return {
        ...
        async signUrl(bucket, key, ttlSec, op) {
            try {
                ...
                const url = await generateBlobSASUrl(blockBlobClient.name, blobService, { permissions, expiresOn: new Date(Date.now() + ttlSec * 1000) });
                return url;
            }
            catch (err) {
