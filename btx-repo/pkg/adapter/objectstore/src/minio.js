...
export function createMinioStore(cfg) {
    const region = cfg.region ?? 'us-east-1';
    const service = 's3';
    const f = cfg.fetch ?? fetch;
    const base = cfg.endpoint.replace(/\/$/, '');
    async function send(method, bucket, key, body, extraHeaders = {}) {
    ...
    return {
        async put(bucket, key, body, opts) {
            const extra = {};
            if (opts?.contentType)
                extra['content-type'] = opts.contentType;
            const res = await send('PUT', bucket, key, body, extra);
            if (!res.ok)
                throw new ObjectStoreError('provider_error', `s3 PUT ${res.status}`);
            return { etag: res.headers.get('etag') ?? '' };
        },
        async get(bucket, key) {
            const res = await send('GET', bucket, key, null);
            if (res.status === 404)
                throw new ObjectStoreError('not_found', `s3 GET ${key}`);
            if (!res.ok)
                throw new ObjectStoreError('provider_error', `s3 GET ${res.status}`);
        ...
        async head(bucket, key) {
            ...
            if (res.status === 404)
                return null;
            if (!res.ok)
                throw new ObjectStoreError('provider_error', `s3 HEAD ${res.status}`);
        ...
        async delete(bucket, key) {
            ...
            if (!res.ok && res.status !== 404)
                throw new ObjectStoreError('provider_error', `s3 DELETE ${res.status}`);
        },
        async healthz() {
            try {
                ...
                return { ok: false, provider: 'minio' };
            }
        }
    };
}
